...
const REPORT_QUERY_BY_KEY = {
  ...
  journal_voucher_register:
    "?voucher_type=journal&report_mode=details&load_report=1",
...
const VOUCHER_PAGES = [
  {
    path: "/vouchers/cash?new=1",
    receiptField: "cash_receipt",
    paymentField: "cash_payment",
    policyKey: "CASH_VOUCHER",
  },
  {
    path: "/vouchers/bank?new=1",
    receiptField: "bank_receipt",
    paymentField: "bank_payment",
    policyKey: "BANK_VOUCHER",
  },
  {
    path: "/vouchers/journal?new=1",
    receiptField: "debit",
    paymentField: "credit",
    policyKey: "JOURNAL_VOUCHER",
  },
];
...
const getReportPath = (reportKey) =>
  `/reports/financial/${reportKey}${REPORT_QUERY_BY_KEY[reportKey] || ""}`;
...
const parseEntityRef = (value) => {
  ...
  if (
    !["ACCOUNT", "PARTY", "LABOUR", "EMPLOYEE"].includes(kind) ||
    !Number.isInteger(id) ||
    id <= 0
  ) {
    return null;
...
const pickVoucherEntityRef = async (page) => {
  const values = await page
    ...
    .evaluateAll((options) =>
      options.map((opt) => String(opt.value || "")).filter(Boolean),
    );
...
const getHeaderAccountIdForVoucher = async (page, voucherTypeCode) => {
  if (voucherTypeCode !== "CASH_VOUCHER" && voucherTypeCode !== "BANK_VOUCHER")
    return null;
...
const submitVoucherWithDeterministicLines = async (page, voucherTypeCode) => {
  ...
  const headerAccountId = await getHeaderAccountIdForVoucher(
    page,
    voucherTypeCode,
  );
  if (
    voucherTypeCode === "CASH_VOUCHER" &&
    (!Number.isInteger(headerAccountId) || headerAccountId <= 0)
  ) {
    return false;
  }
        ...
        ? [
            {
              ...ref,
              bank_payment: 300,
              reference_no: "E2E-MATRIX",
              description: "E2E matrix bank",
            },
          ]
        : [{ ...ref, cash_payment: 300, description: "E2E matrix cash" }];

  const voucherForm = page.locator("form[data-voucher-form]").first();
  const csrfToken = await voucherForm
    .locator('input[name="_csrf"]')
    .inputValue();
  const voucherDate = await voucherForm
    .locator('input[name="voucher_date"]')
    .inputValue();
  const formAction =
    (await voucherForm.getAttribute("action")) || new URL(page.url()).pathname;
  const targetUrl = new URL(
    formAction || new URL(page.url()).pathname,
    page.url(),
  ).toString();
...
const setVoucherPermission = async (userId, scopeKey, permissions = {}) =>
  setUserScopePermission({
    userId,
    scopeType: "VOUCHER",
    scopeKey,
...
test.describe("Financial pages matrix - users, permissions, approvals", () => {
  ...
  test.beforeAll(async () => {
    ...
    const creatorUser = process.env.E2E_FIN_CREATOR_USER || "e2e_fin_creator";
    const creatorPass = process.env.E2E_FIN_CREATOR_PASS || "Creator@123";

    process.env.E2E_LIMITED_USER = limitedUser;
    process.env.E2E_LIMITED_PASS = limitedPass;
    process.env.E2E_FIN_CREATOR_USER = creatorUser;
    process.env.E2E_FIN_CREATOR_PASS = creatorPass;
    ...
    for (const voucherPage of VOUCHER_PAGES) {
      await setVoucherPermission(
        contextState.creatorUserId,
        voucherPage.policyKey,
        {
          can_navigate: true,
          can_view: true,
          can_create: true,
  ...
  test("admin can load all financial report pages", async ({ page }) => {
    ...
    for (const reportKey of REPORT_KEYS) {
      const response = await page.goto(getReportPath(reportKey), {
        waitUntil: "domcontentloaded",
      });
      expect(response?.status(), `admin status for ${reportKey}`).toBe(200);

      await expect(
        page.locator("[data-ledger-filter-form]"),
        `filter form for ${reportKey}`,
      ).toBeVisible();
      ...
      if (reportKey === "expense_trends") {
        ...
        expect(
          (await root.count()) > 0 || (await noEntries.count()) > 0,
        ).toBeTruthy();
      }

      if (
        reportKey === "voucher_register" ||
        reportKey === "bank_transactions"
      ) {
  ...
  test("limited user report access follows permission boundaries", async ({
    page,
  }) => {
    ...
    for (const reportKey of REPORT_KEYS) {
      ...
      if (status === 200) {
        await expect(
          page.locator("[data-ledger-filter-form]"),
          `limited form visible on ${reportKey}`,
        ).toBeVisible();
      } else {
        await expect(
          page.getByText(/permission denied|forbidden/i),
        ).toBeVisible();
      }
    }
  });
  ...
  test("admin can load all financial voucher entry pages", async ({ page }) => {
    ...
    for (const voucherPage of VOUCHER_PAGES) {
      const response = await page.goto(voucherPage.path, {
        waitUntil: "domcontentloaded",
      });
      expect(response?.status(), `admin voucher load ${voucherPage.path}`).toBe(
        200,
      );
  ...
  test("limited user voucher entry routes enforce permission or allow with constraints", async ({
    page,
  }) => {
    await login(page, "E2E_LIMITED");
  ...
  test("non-approver creator flow honors approval-required policy for each financial voucher type", async ({
    page,
  }) => {
    await login(page, "E2E_FIN_CREATOR");
    let executed = 0;
    ...
    for (const voucherPage of VOUCHER_PAGES) {
      ...
      const summary = `ADD ${voucherPage.policyKey}`;
      ...
      const submitted = await submitVoucherWithDeterministicLines(
        page,
        voucherPage.policyKey,
      );
  ...
  test("limited user create flow across vouchers routes to approval or save based on permissions", async ({
    page,
  }) => {
    await login(page, "E2E_LIMITED");
