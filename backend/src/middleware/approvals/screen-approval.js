...
const actionToPermission = (action) =>
  action && action.startsWith("can_") ? action : `can_${action}`;
...
const queueApproval = async ({
  ...
  try {
    ...
    const approvalAction = String(enrichedNewValue?._action || action || "")
      .trim()
      .toLowerCase();
    let requestId = null;
    ...
    if (entityType === "BOM" && approvalAction) {
      let duplicateQuery = knex("erp.approval_request as ar")
        ...
        .andWhereRaw("COALESCE(ar.new_value ->> '_action', '') = ?", [
          approvalAction,
        ])
        .orderBy("ar.id", "desc");
    ...
    notifyPendingApproval({
      ...
      console.error("[screen-approval] in-app notify failed", {
        requestId,
        entityType,
        error: err?.message || err,
      });
    ...
    console.error("[screen-approval] enqueue failed", {
      entityType,
      entityId,
      error: err.message,
    });
...
const handleScreenApproval = async ({
  ...
  if (!allowed && !approvalRequired) {
    debugApproval(
      "[screen-approval] permission missing; routing action to approval queue",
    );
  ...
  if (approvalRequired) {
    ...
    if (res && typeof translator === "function") {
      ...
      setCookie(
        ...
        JSON.stringify({
          message:
            translator("approval_sent") ||
            translator("approval_submitted") ||
  ...
  debugApproval(
    "[screen-approval] approval not required, proceeding without queue",
  );
  return { queued: false };
};
