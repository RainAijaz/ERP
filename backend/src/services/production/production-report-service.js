...
const normalizeReportType = (value) => {
  const normalized = String(value || "details")
    .trim()
    .toLowerCase();
  return normalized === "summary" ? "summary" : "details";
};

const normalizeDepartmentReportOrderBy = (value) => {
  const normalized = String(value || "department")
    .trim()
    .toLowerCase();
  return normalized === "sku" ? "sku" : "department";
};

const normalizeConsumptionReportType = (value) => {
  const normalized = String(value || "SEMIFINISHED")
    .trim()
    .toUpperCase();
  if (["RAWMATERIAL", "RAW_MATERIAL", "RM"].includes(normalized)) {
    return "RAWMATERIAL";
  }
  return "SEMIFINISHED";
};

const normalizeConsumptionReportOrderBy = (
  value,
  consumptionType = "SEMIFINISHED",
) => {
  const normalized = String(value || "department")
    .trim()
    .toLowerCase();
  if (consumptionType === "RAWMATERIAL") {
    if (["raw_material", "rawmaterial", "item"].includes(normalized)) {
      return "raw_material";
    }
    return "department";
  }
...
const getProductionControlReportPageData = async ({ req, input = {} }) => {
  ...
  const selectedLabourIds = normalizeIdList(
    input?.labour_ids || input?.labourIds,
  );
  const selectedProductGroupIds = normalizeIdList(
    input?.product_group_ids || input?.productGroupIds,
  );
  const selectedProductSubgroupIds = normalizeIdList(
    input?.product_subgroup_ids || input?.productSubgroupIds,
  );
  const selectedArticleIds = normalizeIdList(
    input?.article_ids || input?.articleIds,
  );
  const productionType = normalizeProductionType(
    input?.production_type || input?.productionType,
  );
  const selectedSkuUnit = normalizeSkuUnit(input?.sku_unit || input?.skuUnit);
  const stageScope = normalizeStageScope(
    input?.stage_scope || input?.stageScope,
  );
  const orderBy = normalizeProductionControlOrderBy(
    input?.order_by || input?.orderBy,
  );
  const reportType = normalizeReportType(
    input?.report_type || input?.reportType,
  );
  const reportLoaded =
  ...
  const deptExpr = deptExprParts.length
    ? `coalesce(${deptExprParts.join(", ")})`
    : "null::bigint";
  ...
  const stageExpr = stageExprParts.length
    ? `coalesce(${stageExprParts.join(", ")})`
    : "null::bigint";
  ...
  if (lossLineHasDeptId || lossLineHasStageId) {
    query = query.leftJoin(
      "erp.abnormal_loss_line as alln",
      "alln.voucher_line_id",
      "vl.id",
    );
  }
  if (labourVoucherLineHasDeptId) {
    query = query.leftJoin(
      "erp.labour_voucher_line as lvl",
      "lvl.voucher_line_id",
      "vl.id",
    );
  }
  ...
  query = query
    ...
    .select(
      ...
      voucherLineHasLabourId
        ? "vl.labour_id"
        : knex.raw("null::bigint as labour_id"),
      "vl.meta",
      productionLineHasIsPacked
        ? "pl.is_packed as production_is_packed"
        : knex.raw("null::boolean as production_is_packed"),
  ...
  if (selectedBranchIds.length)
    query = query.whereIn("vh.branch_id", selectedBranchIds);
  if (fromDate) query = query.where("vh.voucher_date", ">=", fromDate);
  if (toDate) query = query.where("vh.voucher_date", "<=", toDate);
  ...
  const explicitSkuIds = [
    ...new Set(rawRows.map((row) => toPositiveInt(row.sku_id)).filter(Boolean)),
  ];
  const sourceSkuIds = [
    ...new Set(
      rawRows.map((row) => parseSourceSkuIdFromMeta(row.meta)).filter(Boolean),
    ),
  ];
  const allSkuIds = [...new Set([...explicitSkuIds, ...sourceSkuIds])];

  const skuRows = await (allSkuIds.length
    ? knex("erp.skus as s")
  ...
  let mappedRows = rawRows.map((row) => {
    ...
    const effectiveProductionType = String(sku?.item_type || "")
      .trim()
      .toUpperCase();
    ...
    const lineUnitCode = String(lineMeta.unit || lineMeta.entry_unit || "")
      .trim()
      .toUpperCase();
    const lineStatus = String(lineMeta.status || lineMeta.row_status || "")
      .trim()
      .toUpperCase();
    const unitFromMeta =
      lineUnitCode === "PAIR" || lineUnitCode === "DZN" ? lineUnitCode : "";
    const unitFromStatus =
      lineStatus === "PACKED" ? "DZN" : lineStatus === "LOOSE" ? "PAIR" : "";
    const unitFromProductionExt =
      row.production_is_packed === true
        ? "DZN"
        : row.production_is_packed === false
          ? "PAIR"
          : "";
    const skuUnit =
      unitFromMeta || unitFromStatus || unitFromProductionExt || "PAIR";
    ...
    return {
      ...
      production_type:
        effectiveProductionType === "FG" || effectiveProductionType === "SFG"
  ...
  if (stageScope === "FINAL_ONLY") {
    const skuIdsForStageFilter = [
      ...new Set(
        mappedRows
          .map((row) => toPositiveInt(row.effective_sku_id))
          .filter(Boolean),
      ),
    ];
    ...
    await Promise.all(
      skuIdsForStageFilter.map(async (skuId) => {
        try {
          ...
          const finalRoute = [...candidateRoutes].sort(
  ...
  const rows = mappedRows.filter((row) => {
    if (productionType !== "ALL" && row.production_type !== productionType)
      return false;
    if (
      selectedSkuUnit !== "ALL" &&
      String(row.sku_unit || "").toUpperCase() !== selectedSkuUnit
    )
      return false;
    if (
      selectedArticleIds.length &&
      (!row.article_id || !selectedArticleIds.includes(Number(row.article_id)))
    )
      return false;
    if (
      selectedProductGroupIds.length &&
      (!row.product_group_id ||
        !selectedProductGroupIds.includes(Number(row.product_group_id)))
    )
      return false;
    if (
      selectedProductSubgroupIds.length &&
      (!row.product_subgroup_id ||
        !selectedProductSubgroupIds.includes(Number(row.product_subgroup_id)))
    )
      return false;
    return true;
  });
  ...
  const resolveGroupIdentity = (row) => {
    ...
    if (orderBy === "sku")
      return {
        key: `sku:${String(row.sku_code || "-").trim() || "-"}`,
        label: String(row.sku_code || "-"),
      };
    if (orderBy === "department")
      return {
        key: `department:${String(row.department_name || "-").trim() || "-"}`,
        label: String(row.department_name || "-"),
      };
    return {
      key: `voucher:${Number(row.voucher_id || 0)}`,
  ...
  sortedRows.forEach((row) => {
    ...
    if (String(row.department_name || "").trim())
      existing.department_names.add(String(row.department_name || "").trim());
    if (String(row.labour_name || "").trim())
      existing.labour_names.add(String(row.labour_name || "").trim());
    if (String(row.stage_name || "").trim())
      existing.stage_names.add(String(row.stage_name || "").trim());
    if (String(row.production_type || "").trim())
      existing.production_type_names.add(
        String(row.production_type || "")
          .trim()
          .toUpperCase(),
      );
    if (String(row.sku_unit || "").trim())
      existing.unit_names.add(
        String(row.sku_unit || "")
          .trim()
          .toUpperCase(),
      );
    if (String(row.product_group_name || "").trim())
      existing.product_group_names.add(
        String(row.product_group_name || "").trim(),
      );
    ...
    if (
      ...
      existing.packed_qty =
        Number(existing.packed_qty || 0) + Number(row.qty || 0);
    } else if (
      String(row.sku_unit || "")
        .trim()
        .toUpperCase() === "PAIR"
      ...
      existing.loose_qty =
        Number(existing.loose_qty || 0) + Number(row.qty || 0);
    }
    detailGroupMap.set(identity.key, existing);
  });

  const groups = Array.from(detailGroupMap.values())
    .map((group) => ({
      key: group.key,
      label: group.label,
      rows: group.rows,
  ...
  sortedRows.forEach((row) => {
    ...
    if (rowDate) {
      if (!existing.first_date || rowDate < existing.first_date)
        existing.first_date = rowDate;
      if (!existing.last_date || rowDate > existing.last_date)
        existing.last_date = rowDate;
    }
    existing.qty = Number(existing.qty || 0) + Number(row.qty || 0);
    if (
      String(row.sku_unit || "")
  ...
  const summaryRows = Array.from(summaryMap.values())
    .map((row) => ({
      ...
      branch_label: (() => {
        const names = Array.from(row.branch_names.values()).sort((a, b) =>
          compareText(a, b),
        );
  ...
  const totals = {
    qty: Number(
      sortedRows.reduce((acc, row) => acc + Number(row.qty || 0), 0).toFixed(3),
    ),
    packedQty: Number(
      sortedRows
        .reduce(
    ...
    voucherCount: new Set(sortedRows.map((row) => Number(row.voucher_id || 0)))
      .size,
    branchCount: new Set(
      sortedRows
...
const getProductionDepartmentWipReportPageData = async ({
  req,
  input = {},
}) => {
  const branchOptionsPromise = loadBranchOptions(req);
  ...
  const orderBy = normalizeDepartmentReportOrderBy(
    input?.order_by || input?.orderBy,
  );
  const reportType = normalizeReportType(
    input?.report_type || input?.reportType,
  ...
  const [branchOptions, productGroups, productSubgroups, articles] =
    await Promise.all([
      branchOptionsPromise,
      productGroupsPromise,
      productSubgroupsPromise,
  ...
  if (selectedProductSubgroupIds.length) {
    ledgerQuery = ledgerQuery.whereIn(
      "i.subgroup_id",
      selectedProductSubgroupIds,
    );
  }
  ...
  rawLedgerRows.forEach((row) => {
    ...
    const existing = stageStats.get(key) || {
      closing_pairs: 0,
      last_in_date: null,
      oldest_pending_in_date: null,
      branch_name: String(row.branch_name || "-"),
      fifo_entries: [],
    };
    if (direction === 1) {
      existing.closing_pairs = Number(
        (Number(existing.closing_pairs || 0) + qtyPairs).toFixed(3),
      );
      if (txnDate) {
        existing.fifo_entries.push({
          txn_date: txnDate,
      ...
      if (
        txnDate &&
        (!existing.last_in_date || txnDate > existing.last_in_date)
      ) {
      ...
      let consumePairs = Number(qtyPairs.toFixed(3));
      while (consumePairs > 0 && existing.fifo_entries.length) {
        const head = existing.fifo_entries[0];
        const headQty = Number(Number(head.qty_pairs || 0).toFixed(3));
        const consumed = Math.min(headQty, consumePairs);
        const remainingHead = Number((headQty - consumed).toFixed(3));
        consumePairs = Number((consumePairs - consumed).toFixed(3));
        if (remainingHead <= 0) {
          existing.fifo_entries.shift();
        } else {
          head.qty_pairs = remainingHead;
          break;
        }
      }
    }

    existing.oldest_pending_in_date =
      existing.fifo_entries.length > 0
        ? String(existing.fifo_entries[0]?.txn_date || "").trim() || null
        : null;
  ...
  const skuIds = [
    ...new Set(
      Array.from(branchSkuKeys.values())
        .map((entry) => Number(String(entry).split(":")[1]))
        .filter(Boolean),
    ),
  ];
  ...
  await Promise.all(
    skuIds.map(async (skuId) => {
      try {
        ...
        const stageRoutes = Array.isArray(profile?.stageRoutes)
          ? profile.stageRoutes
          : [];
        if (!stageRoutes.length) return;
        const requiredRoutes = stageRoutes
        ...
        routeChainBySku.set(
          ...
          [...effectiveRoutes]
            ...
            .map((route) => ({
              ...
              dept_id:
                toPositiveInt(route.dept_id) || toPositiveInt(route.stage_id),
  ...
  const deptIds = [
    ...new Set(
      Array.from(routeChainBySku.values())
        .flatMap((routes) =>
          routes.map((route) => toPositiveInt(route.dept_id)),
        )
        .filter(Boolean),
    ),
  ];
  ...
  Array.from(branchSkuKeys.values()).forEach((entry) => {
    ...
    for (let i = 1; i < routes.length; i += 1) {
      ...
      const previousStats = stageStats.get(
        `${branchId}:${skuId}:${Number(previousDeptId)}`,
      );
      const pendingPairs = Number(
        Number(previousStats?.closing_pairs || 0).toFixed(3),
      );
      if (!(pendingPairs > 0)) continue;

      let agingDays = 0;
      const referenceDate =
        previousStats?.oldest_pending_in_date ||
        previousStats?.last_in_date ||
        null;
  ...
  detailRows.sort((a, b) => {
    if (orderBy === "sku") {
      const bySku = String(a.sku_code || "").localeCompare(
        String(b.sku_code || ""),
      );
      if (bySku) return bySku;
      const byPending =
        Number(b.pending_pairs || 0) - Number(a.pending_pairs || 0);
      if (byPending) return byPending;
      const byDept = String(a.department_name || "").localeCompare(
        String(b.department_name || ""),
      );
      if (byDept) return byDept;
      return String(a.branch_name || "").localeCompare(
  ...
  detailRows.forEach((row) => {
    const key =
      orderBy === "sku"
        ? row.sku_id
          ? `sku:${Number(row.sku_id)}`
          : `sku:${String(row.sku_code || "-")}`
        : row.department_id
          ? `dept:${Number(row.department_id)}`
    ...
    existing.pending_pairs =
      Number(existing.pending_pairs || 0) + Number(row.pending_pairs || 0);
    existing.aging_sum =
      Number(existing.aging_sum || 0) + Number(row.aging_days || 0);
    existing.aging_max = Math.max(
      Number(existing.aging_max || 0),
      Number(row.aging_days || 0),
    );
  ...
  const summaryRows = Array.from(summaryMap.values())
    .map((row) => ({
      ...
      pending_departments: row.department_ids.size,
      avg_aging_days:
        row.row_count > 0
          ? Number(
              (Number(row.aging_sum || 0) / Number(row.row_count || 1)).toFixed(
                2,
              ),
            )
          : 0,
  ...
  const totals = {
    pendingPairs: Number(
      detailRows
        .reduce((sum, row) => sum + Number(row.pending_pairs || 0), 0)
        .toFixed(3),
    ),
    pendingDozen: Number(
      detailRows
        .reduce((sum, row) => sum + Number(row.pending_dozen || 0), 0)
        .toFixed(3),
    ),
    articleCount: new Set(
      detailRows.map((row) => Number(row.article_id || 0)).filter(Boolean),
    ).size,
    skuCount: new Set(
      detailRows.map((row) => Number(row.sku_id || 0)).filter(Boolean),
    ).size,
...
const getProductionDepartmentWipBalancesReportPageData = async ({
  req,
  input = {},
}) => {
  const branchOptionsPromise = loadBranchOptions(req);
  ...
  const selectedSkuIds = normalizeIdList(input?.sku_ids || input?.skuIds);
  const orderBy = normalizeDepartmentReportOrderBy(
    input?.order_by || input?.orderBy,
  );
  const reportType = normalizeReportType(
  ...
  const departmentIdSet = new Set(departments.map((row) => Number(row.id)));
  const skuIdSet = new Set(skus.map((row) => Number(row.id)));
  const validDepartmentIds = selectedDepartmentIds.filter((id) =>
    departmentIdSet.has(Number(id)),
  );
  const validSkuIds = selectedSkuIds.filter((id) => skuIdSet.has(Number(id)));
  ...
  if (validDepartmentIds.length) {
    query = query.whereIn("wl.dept_id", validDepartmentIds);
  }
  if (validSkuIds.length) {
    query = query.whereIn("wl.sku_id", validSkuIds);
  }
  ...
  rawRows.forEach((row) => {
    ...
    const direction = Number(row.direction || 0) === 1 ? 1 : -1;
    const signedPairs = Number(
      (direction === 1 ? qtyPairs : -qtyPairs).toFixed(3),
    );
    const txnDate = toDateOnly(row.txn_date);
  ...
  const asOfTs = Date.parse(asOfDate);
  const dayMs = 24 * 60 * 60 * 1000;
  const detailRows = Array.from(groupMap.values())
    .filter((row) => Number(row.closing_pairs || 0) > 0)
    .map((row) => {
      let agingDays = 0;
      const referenceDate =
        row.oldest_pending_in_date || row.last_in_date || null;
      if (referenceDate && Number.isFinite(asOfTs)) {
        const refTs = Date.parse(referenceDate);
        if (Number.isFinite(refTs)) {
          agingDays = Math.max(0, Math.floor((asOfTs - refTs) / dayMs));
      ...
      return {
        ...row,
        aging_days: Number(agingDays || 0),
        closing_pairs: Number(Number(row.closing_pairs || 0).toFixed(3)),
        closing_dozen: Number((Number(row.closing_pairs || 0) / 12).toFixed(3)),
      };
    })
    .sort((a, b) => {
      if (orderBy === "sku") {
        ...
        if (bySku) return bySku;
        const byClosing =
          Number(b.closing_pairs || 0) - Number(a.closing_pairs || 0);
        if (byClosing) return byClosing;
        return String(a.department_name || "").localeCompare(
          String(b.department_name || ""),
        );
      }
  ...
  detailRows.forEach((row) => {
    ...
    if (row.sku_id) existing.sku_ids.add(Number(row.sku_id));
    if (row.department_id)
      existing.department_ids.add(Number(row.department_id));
    if (
      String(row.branch_name || "").trim() &&
    ...
    existing.closing_pairs =
      Number(existing.closing_pairs || 0) + Number(row.closing_pairs || 0);
    existing.aging_sum =
      Number(existing.aging_sum || 0) + Number(row.aging_days || 0);
    existing.aging_max = Math.max(
      Number(existing.aging_max || 0),
  ...
  const summaryRows = Array.from(summaryMap.values())
    .map((row) => ({
      ...
      sku_id: row.sku_id,
      sku_code: row.sku_code,
      sku_count: row.sku_ids.size,
      department_count: row.department_ids.size,
      avg_aging_days:
        row.row_count > 0
          ? Number(
              (Number(row.aging_sum || 0) / Number(row.row_count || 1)).toFixed(
      ...
      closing_pairs: Number(Number(row.closing_pairs || 0).toFixed(3)),
      closing_dozen: Number((Number(row.closing_pairs || 0) / 12).toFixed(3)),
      branch_label: summarizeNameSet(row.branch_names),
    }))
    .sort((a, b) => {
      const byClosing =
        Number(b.closing_pairs || 0) - Number(a.closing_pairs || 0);
  ...
  const totals = {
    closingPairs: Number(
      detailRows
        .reduce((sum, row) => sum + Number(row.closing_pairs || 0), 0)
        .toFixed(3),
    ),
    closingDozen: Number(
      detailRows
        .reduce((sum, row) => sum + Number(row.closing_dozen || 0), 0)
        .toFixed(3),
    ),
    skuCount: new Set(
      detailRows.map((row) => Number(row.sku_id || 0)).filter(Boolean),
...
const getProductionDepartmentWipLedgerReportPageData = async ({
  ...
  const missingRequiredFilters =
    reportLoaded && (!validDepartmentId || !validSkuId);
  ...
  rawRows.forEach((row) => {
    ...
    if (
      direction === 1 &&
      txnDate &&
      (!group.last_in_date || txnDate > group.last_in_date)
    ) {
    ...
    group.transactions.push({
      ...
      voucher_type_code: String(row.voucher_type_code || "")
        .trim()
        .toUpperCase(),
      source_voucher_status: String(row.source_voucher_status || "")
        .trim()
        .toUpperCase(),
    });
    groupMap.set(key, group);
  ...
  groupMap.forEach((group) => {
    ...
    const transactionRows = (group.transactions || []).map((row) => {
      runningPairs = Number(
        (runningPairs + Number(row.signed_pairs || 0)).toFixed(3),
      );
      return {
        ...row,
  ...
  const totals = detailGroups.reduce(
    (acc, group) => ({
      openingPairs: Number(
        (
          Number(acc.openingPairs || 0) + Number(group.opening_pairs || 0)
        ).toFixed(3),
      ),
      inPairs: Number(
      ...
      closingPairs: Number(
        (
          Number(acc.closingPairs || 0) + Number(group.closing_pairs || 0)
        ).toFixed(3),
      ),
      closingDozen: Number(
        (
          Number(acc.closingDozen || 0) + Number(group.closing_dozen || 0)
        ).toFixed(3),
      ),
...
const getProductionConsumptionReportPageData = async ({ req, input = {} }) => {
  ...
  const consumptionType = normalizeConsumptionReportType(
    input?.type || input?.consumption_type || input?.consumptionType,
  );
  const orderBy = normalizeConsumptionReportOrderBy(
    input?.order_by || input?.orderBy,
    consumptionType,
  );
  const reportType = normalizeReportType(
  ...
  const productGroupsPromise = knex("erp.product_groups as pg")
    ...
    .whereExists(function whereGroupHasConsumableItems() {
      this.select(1)
        .from("erp.items as i")
        .whereRaw("i.group_id = pg.id")
        .andWhere("i.is_active", true)
        .whereIn("i.item_type", stockItemTypes);
    })
    .orderBy("pg.name", "asc");
  const productSubgroupsPromise = knex("erp.product_subgroups as sg")
    .select("sg.id", "sg.name", "sg.group_id")
    .where({ "sg.is_active": true })
    .whereExists(function whereSubgroupHasConsumableItems() {
      this.select(1)
        .from("erp.items as i")
        .whereRaw("i.subgroup_id = sg.id")
        .andWhere("i.is_active", true)
  ...
  const [branchOptions, departments, productGroups, productSubgroups] =
    await Promise.all([
      branchOptionsPromise,
      departmentsPromise,
      productGroupsPromise,
  ...
  const options = {
    ...
    productSubgroups,
    consumptionTypes: [
      { value: "SEMIFINISHED", labelKey: "semi_finished" },
      { value: "RAWMATERIAL", labelKey: "raw_material" },
    ],
    orderByModes:
      consumptionType === "RAWMATERIAL"
        ? [
            { value: "department", labelKey: "department" },
            { value: "raw_material", labelKey: "raw_material" },
          ]
        : [
            { value: "department", labelKey: "department" },
            { value: "sku", labelKey: "sku" },
  ...
  let query = knex("erp.voucher_header as vh")
    .join("erp.voucher_line as vl", "vl.voucher_header_id", "vh.id")
    .leftJoin("erp.consumption_header as ch", "ch.voucher_id", "vh.id")
    .leftJoin("erp.voucher_header as src", "src.id", "ch.source_production_id")
    .leftJoin("erp.dcv_header as dh", "dh.voucher_id", "src.id")
    .leftJoin("erp.branches as b", "b.id", "vh.branch_id")
    .select(
      "vh.id as voucher_id",
      "vh.voucher_no",
      ...
      "vl.qty",
      "vl.meta",
      "src.voucher_no as source_voucher_no",
      "dh.dept_id as source_dept_id",
    )
    .where({
      "vh.voucher_type_code": PRODUCTION_VOUCHER_TYPES.consumption,
      "vh.status": "APPROVED",
    });
  ...
  if (toDate) query = query.where("vh.voucher_date", "<=", toDate);
  if (consumptionType === "RAWMATERIAL") {
    query = query.where("vl.line_kind", "ITEM");
  } else {
    query = query.where("vl.line_kind", "SKU");
  }
  ...
  const metaRows = rawRows.map((row) => ({
    ...row,
    metaObj: toMetaObject(row.meta),
  }));

  const skuIds = [
    ...new Set(
      metaRows
        .map((row) => toPositiveInt(row.sku_id))
        .filter(Boolean),
    ),
  ];
  const sourceSkuIds = [
    ...new Set(
      metaRows
        .map((row) => toPositiveInt(row.metaObj?.source_sku_id))
        .filter(Boolean),
    ),
  ];
  const allSkuIds = [...new Set([...skuIds, ...sourceSkuIds])];
  const itemIds = [
    ...new Set(
      metaRows
  ...
  const uomIds = [
    ...new Set(
      metaRows
        .map((row) => toPositiveInt(row.uom_id))
        .filter(Boolean),
    ),
  ];
  const departmentIdsFromRows = [
    ...new Set(
      metaRows
        .map((row) =>
          toPositiveInt(
            row.metaObj?.department_id ||
              row.metaObj?.dept_id ||
              row.source_dept_id,
          ),
        )
  ...
  const [skuRows, itemRows, uomRows, departmentRows] = await Promise.all([
    allSkuIds.length
      ? knex("erp.skus as s")
          .join("erp.variants as v", "v.id", "s.variant_id")
          .join("erp.items as i", "i.id", "v.item_id")
  ...
  const skuById = new Map(skuRows.map((row) => [Number(row.sku_id), row]));
  const itemById = new Map(itemRows.map((row) => [Number(row.id), row]));
  const uomCodeById = new Map(
    uomRows.map((row) => [Number(row.id), String(row.code || "")]),
  );
  const departmentNameById = new Map(
    departmentRows.map((row) => [Number(row.id), String(row.name || "-")]),
  );

  const rows = metaRows
    .map((row) => {
      const departmentId = toPositiveInt(
        row.metaObj?.department_id || row.metaObj?.dept_id || row.source_dept_id,
      );
      const departmentName = departmentId
        ? departmentNameById.get(Number(departmentId)) || "-"
        : "-";
      const sourceSkuId = toPositiveInt(row.metaObj?.source_sku_id);
      const sourceSku = sourceSkuId
        ? skuById.get(Number(sourceSkuId)) || null
        : null;
      const base = {
        voucher_id: Number(row.voucher_id || 0),
        line_id: Number(row.line_id || 0),
        line_no: Number(row.line_no || 0),
        voucher_no: Number(row.voucher_no || 0),
        date: toDateOnly(row.voucher_date) || "",
        branch_id: Number(row.branch_id || 0),
        branch_name: String(row.branch_name || "-"),
        source_voucher_no: Number(row.source_voucher_no || 0) || null,
        department_id: departmentId,
        department_name: departmentName,
        qty: Number(Number(row.qty || 0).toFixed(3)),
        uom_code: uomCodeById.get(toPositiveInt(row.uom_id)) || "",
        ...
        item_label: "-",
        article_name: "-",
        source_sku_code: sourceSku?.sku_code || "-",
      };

      if (consumptionType === "RAWMATERIAL") {
        const itemId = toPositiveInt(row.item_id);
        const item = itemId ? itemById.get(Number(itemId)) || null : null;
        return {
          ...base,
          item_id: itemId,
          item_label: String(item?.name || "-"),
          article_name: sourceSku?.item_name || "-",
          product_group_id: toPositiveInt(item?.group_id),
          product_group_name: String(item?.group_name || "-"),
          product_subgroup_id: toPositiveInt(item?.subgroup_id),
          product_subgroup_name: String(item?.subgroup_name || "-"),
        };
      }

      const skuId = toPositiveInt(row.sku_id);
      const sku = skuId ? skuById.get(Number(skuId)) || null : null;
      return {
        ...base,
        item_id: skuId,
        item_label: String(sku?.sku_code || "-"),
        article_name: String(sku?.item_name || "-"),
        product_group_id: toPositiveInt(sku?.group_id),
        product_group_name: String(sku?.group_name || "-"),
        product_subgroup_id: toPositiveInt(sku?.subgroup_id),
        product_subgroup_name: String(sku?.subgroup_name || "-"),
      };
    })
    .filter((row) => {
      if (
        ...
        (!row.department_id ||
          !selectedDepartmentIds.includes(Number(row.department_id)))
      ) {
        return false;
      }
      if (
  ...
  const sortedRows = [...rows].sort((a, b) => {
    if (orderBy === "department") {
      const byDepartment = compareText(a.department_name, b.department_name);
      if (byDepartment) return byDepartment;
    } else {
      const byItem = compareText(a.item_label, b.item_label);
  ...
  const detailRows = sortedRows.map((row) => ({
    ...
    order_group_key:
      orderBy === "department"
        ? `department:${Number(row.department_id || 0) || String(row.department_name || "-")}`
        : `item:${Number(row.item_id || 0) || String(row.item_label || "-")}`,
    order_group_label:
      orderBy === "department" ? row.department_name : row.item_label,
  }));
  ...
  const summaryMap = new Map();
  detailRows.forEach((row) => {
    const existing = summaryMap.get(row.order_group_key) || {
      group_key: row.order_group_key,
      group_label: row.order_group_label,
      voucher_ids: new Set(),
      branch_names: new Set(),
      department_ids: new Set(),
      item_ids: new Set(),
    ...
    if (row.department_id) existing.department_ids.add(Number(row.department_id));
    if (row.item_id) existing.item_ids.add(Number(row.item_id));
    existing.qty = Number(existing.qty || 0) + Number(row.qty || 0);
    existing.line_count += 1;
    summaryMap.set(row.order_group_key, existing);
  });
  ...
  const summaryRows = Array.from(summaryMap.values())
    .map((row) => ({
      ...
      branch_label: summarizeNameSet(row.branch_names),
      department_count: row.department_ids.size,
      item_count: row.item_ids.size,
      line_count: Number(row.line_count || 0),
      qty: Number(Number(row.qty || 0).toFixed(3)),
    }))
    .sort((a, b) => compareText(a.group_label, b.group_label));

  const totals = {
    qty: Number(
      detailRows.reduce((acc, row) => acc + Number(row.qty || 0), 0).toFixed(3),
    ),
    lineCount: detailRows.length,
    voucherCount: new Set(detailRows.map((row) => Number(row.voucher_id || 0)))
      .size,
    branchCount: new Set(
      detailRows
        .map((row) => String(row.branch_name || "").trim())
    ...
    departmentCount: new Set(
      detailRows
        .map((row) => Number(row.department_id || 0))
        .filter((id) => id > 0),
    ).size,
    itemCount: new Set(
      detailRows
...
const getProductionPlannedConsumptionReportPageData = async ({
  req,
  input = {},
}) => {
  ...
  const skuIds = [
    ...new Set(
      planLines.map((row) => toPositiveInt(row?.sku_id)).filter(Boolean),
    ),
  ];
  ...
  for (const line of planLines) {
    ...
    const outputQtyInPairs = Number(
      (
        Number(bom.outputQty || 0) * Number(bom.outputUomFactorToPair || 0)
      ).toFixed(6),
    );
    ...
    for (const rmLine of Array.isArray(bom.rmLines) ? bom.rmLines : []) {
      const override =
        bom?.skuOverrideByRmDept?.get(
          `${Number(rmLine.rm_item_id)}:${Number(rmLine.dept_id)}`,
        ) || null;
      ...
      const uomId =
        toPositiveInt(override?.override_uom_id) ||
        toPositiveInt(rmLine.uom_id);
  ...
  const itemIds = [
    ...new Set(plannedRows.map((row) => Number(row.item_id)).filter(Boolean)),
  ];
  const uomIds = [
    ...new Set(
      plannedRows.map((row) => toPositiveInt(row.uom_id)).filter(Boolean),
    ),
  ];
  ...
  const itemNameById = new Map(
    itemRows.map((row) => [Number(row.id), String(row.name || `#${row.id}`)]),
  );
  const uomCodeById = new Map(
    uomRows.map((row) => [Number(row.id), String(row.code || "")]),
  );
  const availableByItem = new Map(
    availableRows.map((row) => [Number(row.item_id), Number(row.qty || 0)]),
  );
  ...
  const rows = plannedRows
    .map((row) => {
      ...
      const availableQty = Number(
        Number(availableByItem.get(Number(row.item_id)) || 0).toFixed(3),
      );
      const shortQty = Number(
        Math.max(0, plannedQty - availableQty).toFixed(3),
      );
      const coveragePct =
        plannedQty > 0
  ...
  const totals = rows.reduce(
    (acc, row) => ({
      plannedQty: Number(
        (acc.plannedQty + Number(row.planned_qty || 0)).toFixed(3),
      ),
      availableQty: Number(
        (acc.availableQty + Number(row.available_qty || 0)).toFixed(3),
      ),
