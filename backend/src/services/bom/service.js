...
const ensureActiveStageMappingsByDepartment = async (
  db,
  deptIds,
  options = {},
) => {
  ...
  (stageRows || []).forEach((row) => {
    ...
    if (!inactiveStageIdByDeptId.has(deptId))
      inactiveStageIdByDeptId.set(deptId, stageId);
  });

  const missingAfterActive = normalizedDeptIds.filter(
    (deptId) => !stageIdByDeptId.has(deptId),
  );
  ...
  const remainingMissingDeptIds = normalizedDeptIds.filter(
    (deptId) => !stageIdByDeptId.has(deptId),
  );
...
const validateRequiredRates = async (db, rmLines, t) => {
  ...
  const itemIds = [
    ...new Set(lineKeys.map((entry) => entry.itemId).filter(Boolean)),
  ];
  if (!itemIds.length) return;

  const [rateRows, itemRows] = await Promise.all([
    db("erp.rm_purchase_rates")
      .select("rm_item_id", "color_id", "size_id")
      .whereIn("rm_item_id", itemIds)
      .andWhere({ is_active: true }),
  ...
  const itemMap = new Map(
    itemRows.map((row) => [toNumberOrNull(row.id), row.name]),
  );
...
const validateAndNormalizeInput = async (db, input, t, options = {}) => {
  const enforceSkuRuleCompleteness = Boolean(
    options?.enforceSkuRuleCompleteness,
  );
  const enforceRmVariantIdentity = options?.enforceRmVariantIdentity !== false;
  const details = [];
  ...
  const item = itemId
    ? await db("erp.items")
        .select("id", "name", "item_type", "base_uom_id")
        .where({ id: itemId })
        .first()
    : null;
  if (!item) {
  ...
  if (
    item?.base_uom_id &&
    outputUomId &&
    Number(outputUomId) !== Number(item.base_uom_id)
  ) {
    ...
    if (!hasUomConversionsTable) {
      ...
      const uomNameById = new Map(
        (uomRows || []).map((row) => [
          Number(row.id),
          String(row.name || row.id),
        ]),
      );
      const outputUomLabel =
        uomNameById.get(Number(outputUomId)) || `UOM ${outputUomId}`;
      const baseUomLabel =
        uomNameById.get(Number(item.base_uom_id)) || `UOM ${item.base_uom_id}`;
  ...
  const rmLines = toArray(input?.rm_lines)
    ...
    .filter(
      (line) =>
        line.rm_item_id || line.color_id || line.size_id || line.dept_id,
    );

  const rmItemIds = [
    ...new Set(rmLines.map((line) => line.rm_item_id).filter(Boolean)),
  ];
  const [rmItems, rmRateVariantRows] = rmItemIds.length
    ? await Promise.all([
        db("erp.items")
          .select("id", "name", "item_type", "base_uom_id")
          .whereIn("id", rmItemIds),
        db("erp.rm_purchase_rates as r")
          .select("r.rm_item_id", "r.color_id", "r.size_id")
          .whereIn("r.rm_item_id", rmItemIds)
          .andWhere("r.is_active", true),
      ])
    : [[], []];
  const rmItemMap = new Map(
    rmItems.map((row) => [toNumberOrNull(row.id), row]),
  );
  const rmVariantRequirementMap = new Map();
  (rmRateVariantRows || []).forEach((row) => {
    const itemId = toNumberOrNull(row?.rm_item_id);
    if (!itemId) return;
    if (!rmVariantRequirementMap.has(itemId)) {
      rmVariantRequirementMap.set(itemId, {
        colorIds: new Set(),
        sizeIds: new Set(),
      });
    }
    const current = rmVariantRequirementMap.get(itemId);
    const colorId = toNumberOrNull(row?.color_id);
    const sizeId = toNumberOrNull(row?.size_id);
    if (colorId) current.colorIds.add(Number(colorId));
  ...
  rmLines.forEach((line, idx) => {
    ...
    if (enforceRmVariantIdentity) {
      ...
      if (
        requiresColor &&
        line.color_id &&
        !variantReq.colorIds.has(Number(line.color_id))
      ) {
        details.push({
          field: "rm_lines_json",
          message: `${rowLabel}: ${
      ...
      if (
        requiresSize &&
        line.size_id &&
        !variantReq.sizeIds.has(Number(line.size_id))
      ) {
        details.push({
          field: "rm_lines_json",
          message: `${rowLabel}: ${
  ...
  const skuIds = [
    ...new Set(sfgLines.map((line) => line.sfg_sku_id).filter(Boolean)),
  ];
  ...
  for (let idx = 0; idx < sfgLines.length; idx += 1) {
    ...
    const approvedBomId =
      approvedBomByItemId.get(toNumberOrNull(sku.item_id)) || null;
  ...
  const labourIds = [
    ...new Set(
      labourLines.map((line) => toNumberOrNull(line.labour_id)).filter(Boolean),
    ),
  ];
  ...
  const labourRows = labourIds.length
    ? await db("erp.labours").select("id", "dept_id").whereIn("id", labourIds)
    : [];
  const labourMapRows =
    hasLabourDeptTable && labourIds.length
      ? await db("erp.labour_department")
          .select("labour_id", "dept_id")
  ...
  labourMapRows.forEach((row) => {
    ...
    if (!labourAllowedDeptMap.has(labourId))
      labourAllowedDeptMap.set(labourId, new Set());
  ...
  const enforceProductionDepartment = Boolean(
    hasAnyActiveProductionDepartment?.id,
  );
  const departmentMap = new Map(
    departmentRows.map((row) => [toNumberOrNull(row.id), row]),
  );
  ...
  rmLines.forEach((line, idx) => {
    ...
    if (
      !dept ||
      !dept.is_active ||
      (enforceProductionDepartment && !dept.is_production)
    ) {
      details.push({
        field: "rm_lines_json",
  ...
  labourLines.forEach((line, idx) => {
    ...
    const sizeKey =
      line.size_scope === "SPECIFIC" ? String(line.size_id || "") : "ALL";
  ...
  const headerSkuRows = itemId
    ...
    : [];
  const headerSkuSet = new Set(
    (headerSkuRows || []).map((row) => toNumberOrNull(row?.id)).filter(Boolean),
  );
  const headerSkuCodeById = new Map(
    (headerSkuRows || []).map((row) => [
      toNumberOrNull(row?.id),
      String(row?.sku_code || row?.id || "").trim(),
    ]),
  );
  const rmComboSet = new Set(
    rmLines.map((line) => `${line.rm_item_id}:${line.dept_id}`),
  );
  ...
  if (rmLines.length && enforceSkuRuleCompleteness) {
    ...
    if (!skuRules.length) {
      details.push({
        field: "sku_rules_json",
        message:
          "Enter required quantity in SKU Rules for all SKU/material rows.",
      });
    }
  }
  ...
  skuRules.forEach((line, idx) => {
    ...
    aggregatedQtyByRmCombo.set(
      rmComboKey,
      Number(aggregatedQtyByRmCombo.get(rmComboKey) || 0) +
        Number(line.required_qty || 0),
    );
  });
    ...
    if (missingSkuRulePairs.length) {
      const sample = missingSkuRulePairs
        ...
        .map((row) => {
          ...
          const skuLabel =
            headerSkuCodeById.get(row.skuId) || `SKU ${row.skuId}`;
          const rmName = String(
            rmItemMap.get(toNumberOrNull(rmItemId))?.name || rmItemId || "",
          ).trim();
  ...
  const hasBomStageRoutingTable = await tableExists(
    db,
    "erp.bom_stage_routing",
  );
  if (
    stageRoutes.length &&
    (!hasProductionStageTable || !hasBomStageRoutingTable)
  ) {
    details.push({
  ...
  if (stageRoutes.length && hasProductionStageTable) {
    const stageRouteDeptIds = [
      ...new Set(stageRoutes.map((line) => line.dept_id).filter(Boolean)),
    ];
    const stageIdByDeptId = await ensureActiveStageMappingsByDepartment(
      db,
      stageRouteDeptIds,
      {
    ...
    const stageIds = [
      ...new Set(stageRoutes.map((line) => line.stage_id).filter(Boolean)),
    ];
    ...
    stageRoutes.forEach((line) => {
      if (
        !line.dept_id &&
        line.stage_id &&
        stageDeptIdByStageId.has(line.stage_id)
      ) {
    ...
    stageRoutes.forEach((line, idx) => {
      ...
      if (
        !activeStageMap.has(line.stage_id) ||
        !activeStageMap.get(line.stage_id)
      ) {
        details.push({
          field: "stage_routes_json",
  ...
  if (sfgLines.length) {
    ...
    sfgLines.forEach((line, idx) => {
      ...
      if (
        mappedStageIds.size &&
        !mappedStageIds.has(Number(line.consumed_in_stage_id))
      ) {
        details.push({
          field: "sfg_lines_json",
...
const ensureDraftUniqueness = async (
  db,
  { itemId, level, excludeId, createdBy, t },
) => {
  let query = db("erp.bom_header")
    .select("id", "bom_no")
    .where({ item_id: itemId, level, status: "DRAFT" });
  const normalizedCreatedBy = toPositiveInt(createdBy);
  if (normalizedCreatedBy)
    query = query.andWhere("created_by", normalizedCreatedBy);
...
const ensureNoExistingBomForItem = async (
  db,
  { itemId, level, excludeId, t },
) => {
  let query = db("erp.bom_header")
    .select("id")
  ...
  const normalizedLevel = String(level || "")
    .trim()
    .toUpperCase();
  if (BOM_LEVELS.has(normalizedLevel))
    query = query.andWhere("level", normalizedLevel);
  if (excludeId) query = query.andWhereNot({ id: excludeId });
  const existing = await query.first();
  if (existing) {
    const message =
...
const snapshotSignature = (snapshot = {}) =>
  JSON.stringify(toSortedObject(buildApprovalSnapshot(snapshot)));
...
const hasPendingApprovalForBomTx = async (db, bomId, options = {}) => {
  ...
  if (actions.length) {
    query = query.whereIn(
      db.raw("COALESCE(new_value ->> '_action', '')"),
      actions,
    );
  }
...
const hasPendingApprovalForBomTargetTx = async (
  db,
  { bomId = null, itemId = null, actions = [] } = {},
) => {
  ...
  let query = db("erp.approval_request as ar").select("ar.id").where({
    "ar.entity_type": "BOM",
    "ar.status": "PENDING",
  });
  ...
  if (normalizedActions.length) {
    query = query.whereIn(
      db.raw("COALESCE(ar.new_value ->> '_action', '')"),
      normalizedActions,
    );
  }
  ...
  if (normalizedItemId) {
    query = query
      .joinRaw(
        "JOIN erp.bom_header bh ON ar.entity_id ~ '^[0-9]+$' AND bh.id = ar.entity_id::bigint",
      )
...
const setBomPending = async (knex, params) =>
  knex.transaction((trx) => setBomPendingTx(trx, params));
...
const toggleBomLifecycleTx = async (trx, { bomId, isActive, t }) => {
  ...
  await trx("erp.bom_header").where({ id }).update({ is_active: nextActive });
  return { id, is_active: nextActive };
};

const toggleBomLifecycle = async (knex, params) =>
  knex.transaction((trx) => toggleBomLifecycleTx(trx, params));
...
const deleteDraftBom = async (knex, params) =>
  knex.transaction((trx) => deleteDraftBomTx(trx, params));

const validateDraftReadyForApproval = async (
  db,
  { bomId, t, locale = "en", intent = "send" } = {},
) => {
  ...
  await validateAndNormalizeInput(
    ...
    {
      header: {
        item_id: toNumberOrNull(form?.header?.item_id),
        level: String(form?.header?.level || "")
          .trim()
          .toUpperCase(),
        output_qty: form?.header?.output_qty,
        output_uom_id: toNumberOrNull(form?.header?.output_uom_id),
      },
  ...
  if (!itemId) {
    throw makeValidationError(
      (t && t("bom_error_item_required")) || "Please select an item.",
    );
  }
  ...
  const skuRows = Array.isArray(options?.itemSkuMap?.[String(itemId)])
    ? options.itemSkuMap[String(itemId)].filter((sku) =>
        toNumberOrNull(sku?.size_id),
      )
    : [];
  ...
  if (hasBomSkuOverrideTable && rmLines.length && skuRows.length) {
    ...
    if (missingPairs.length) {
      const skuCodeById = new Map(
        (options?.itemSkuMap?.[String(itemId)] || []).map((row) => [
          toNumberOrNull(row?.id),
          String(row?.sku_code || row?.id || "").trim(),
        ]),
      );
      const rmNameById = new Map(
        (options?.rmItems || []).map((row) => [
          toNumberOrNull(row?.id),
      ...
      const sample = missingPairs
        ...
        .map(
          (pair) =>
            `${skuCodeById.get(pair.skuId) || `SKU ${pair.skuId}`} / ${rmNameById.get(pair.rmItemId) || `RM ${pair.rmItemId}`} / Dept ${pair.deptId}`,
        )
        .join(", ");
      const suffix =
        missingPairs.length > 12 ? ` (+${missingPairs.length - 12})` : "";
      details.push({
        field: "sku_rules_json",
        message: `Issue in Raw Materials / SKU Rules section: ${
          isApproveIntent
            ? resolveText(
                "bom_error_approval_missing_sku_rules_approve",
                "Complete all SKU Rules rows (quantity and required color variant) before approval.",
              )
            : resolveText(
    ...
    const incompleteRmRows = rmLines.filter(
      (line) => !Number.isFinite(Number(line.qty)) || Number(line.qty) < 0,
    );
  ...
  if (
    departmentsUsedInBom.length &&
    hasProductionStageTable &&
    hasBomStageRoutingTable
  ) {
    const stageRouteIds = [
      ...new Set(
    ...
    const missingDeptIds = departmentsUsedInBom.filter(
      (deptId) => !stageDeptSet.has(deptId),
    );
    if (missingDeptIds.length) {
      ...
      const sample = missingDeptIds
        ...
        .join(", ");
      const suffix =
        missingDeptIds.length > 12 ? ` (+${missingDeptIds.length - 12})` : "";
      details.push({
        field: "stage_routes_json",
        message:
  ...
  if (
    ...
    if (selectedSfgSkuIds.length) {
      ...
      const skuRowById = new Map(
        selectedSfgSkuRows.map((row) => [toNumberOrNull(row.id), row]),
      );
      ...
      sfgRowsForApproval.forEach((line) => {
        ...
        const hasApprovedBom = Boolean(
          itemId && itemApprovedBomMap.get(itemId),
        );
        if (hasApprovedBom) return;
        const skuLabel = String(
          skuRow?.sku_code || line.sfg_sku_id || "",
        ).trim();
        details.push({
          field: "sfg_lines_json",
  ...
  if (details.length) {
    throw makeValidationError(
      isApproveIntent
        ? resolveText(
            "bom_error_approve_blocked",
            "BOM cannot be approved yet. Resolve the issues below.",
          )
        : resolveText(
            "bom_error_approval_blocked",
            "BOM cannot be sent for approval yet. Resolve the issues below.",
          ),
      details,
    );
  }
...
const saveBomDraftTx = async (trx, { input, bomId, userId, requestId, t }) => {
  ...
  if (!existingId) {
    ...
    const current = await trx("erp.bom_header")
      .select("id", "version_no", "bom_no", "status")
      .where({ id: existingId })
      .first();
    if (!current) {
      throw makeValidationError(t("error_not_found") );
...
const saveBomDraft = async (knex, params) => {
  try {
    ...
    const hasConstraint = (key) =>
      constraint.includes(key) || messageText.includes(key);
    if (isUniqueViolation) {
      if (hasConstraint("ux_bom_header_single_draft")) {
        throw makeValidationError(
          (params?.t && params.t("bom_error_draft_exists")) ||
      ...
      if (
        hasConstraint(
          "bom_rm_line_bom_id_rm_item_id_dept_id_color_id_size_id_key",
        )
      ) {
        throw makeValidationError(
          (params?.t && params.t("bom_error_rm_department_duplicate")) ||
      ...
      if (
        hasConstraint(
          "bom_labour_line_bom_id_dept_id_labour_id_size_scope_size_id_rate_type_key",
        )
      ) {
        throw makeValidationError(
          (params?.t && params.t("bom_error_labour_department_duplicate")) ||
          ...
          [
            {
              ...
              message:
                (params?.t &&
                  params.t("bom_error_labour_department_duplicate")) ||
                "This labour is already added for the selected department. Choose a different labour or department.",
            },
          ],
        );
...
const approveBomDirect = async (knex, params) =>
  knex.transaction((trx) => approveBomDirectTx(trx, params));

const createNewVersionFromApprovedTx = async (
  trx,
  { sourceBomId, userId, t },
) => {
  const lifecycleSupported = await hasBomLifecycleColumn(trx);
  ...
  const source = await trx("erp.bom_header")
    .select(sourceFields)
    .where({ id: sourceId })
    .first();
  if (!source)
  ...
  await trx.raw(
    "SELECT id FROM erp.bom_header WHERE item_id = ? AND level = ? FOR UPDATE",
    [source.item_id, source.level],
  );
  await ensureDraftUniqueness(trx, {
...
const createNewVersionFromApproved = async (knex, params) =>
  knex.transaction((trx) => createNewVersionFromApprovedTx(trx, params));

const hasPendingApprovalForBom = async (knex, bomId, options = {}) =>
  hasPendingApprovalForBomTx(knex, bomId, options);
const hasPendingApprovalForBomTarget = async (knex, params = {}) =>
  hasPendingApprovalForBomTargetTx(knex, params);
const findDraftBomByItemLevel = async (
  knex,
  { itemId, level, excludeBomId = null, createdBy = null } = {},
) => {
...
const loadFormOptions = async (knex, locale = "en", options = {}) => {
  ...
  const [
    ...
    ? await Promise.all([
        ...
        knex.schema
          .withSchema("erp")
          .hasColumn("labour_rate_rules", "applies_to_all_labours"),
        knex.schema
          .withSchema("erp")
  ...
  const [
    ...
    knex("erp.rm_purchase_rates as r")
      .select(
        ...
        useUr
          ? knex.raw("COALESCE(s.name_ur, s.name) as size_name")
          : "s.name as size_name",
      )
    ...
    hasSizeItemTypesTable
      ? knex("erp.size_item_types")
          ...
          .whereIn("item_type", ["FG", "SFG"])
      : Promise.resolve([]),
    knex("erp.item_usage").select("fg_item_id", "sfg_item_id"),
    hasLabourRateRulesTable
      ? (() => {
          let query = knex("erp.labour_rate_rules as r")
            .select(
              ...
              hasLabourRateRuleArticleTypeColumn
                ? "r.article_type"
                : knex.raw("NULL::text as article_type"),
            )
          ...
          if (hasLabourRateRuleStatusColumn) {
            query = query.whereRaw(
              "lower(trim(COALESCE(r.status, ''))) = 'active'",
            );
          }
    ...
    hasProductionStagesTable
      ? knex("erp.production_stages as ps")
          ...
          .select(
            ...
            "ps.dept_id",
            useUr
              ? knex.raw("COALESCE(ps.name_ur, ps.name) as name")
              : "ps.name as name",
            useUr
          ...
          .where((builder) => {
            ...
            if (requesterUserId) {
              builder.orWhere((draftBuilder) => {
                draftBuilder
                  .where("status", "DRAFT")
                  .andWhere("created_by", requesterUserId);
              });
            }
          })
      : Promise.resolve([]),
  ...
  const fallbackActiveDeptSet = new Set(
    (departments || []).map((row) => Number(row.id)).filter(Boolean),
  );
  const productionDeptSet = strictProductionDeptSet.size
    ? strictProductionDeptSet
    : fallbackActiveDeptSet;
  const labourDeptMap = {};
  ...
  (labourDeptRows || []).forEach((row) => {
    ...
    if (!labourDeptMap[key].includes(String(deptId)))
      labourDeptMap[key].push(String(deptId));
  });
  (labours || []).forEach((labour) => {
    const labourId = Number(labour.id);
    const fallbackDeptId = Number(labour.dept_id);
    if (!labourId || !fallbackDeptId || !productionDeptSet.has(fallbackDeptId))
      return;
    const key = String(labourId);
    if (!Array.isArray(labourDeptMap[key])) labourDeptMap[key] = [];
    if (!labourDeptMap[key].includes(String(fallbackDeptId)))
      labourDeptMap[key].push(String(fallbackDeptId));
  });
  ...
  (rmRateVariants || []).forEach((row) => {
    ...
    if (
      colorId &&
      !rmItemColorMap[key].some((entry) => Number(entry.id) === colorId)
    ) {
      rmItemColorMap[key].push({
        id: colorId,
        name: row.color_name || String(colorId),
      });
    }
    if (
      sizeId &&
      !rmItemSizeMap[key].some((entry) => Number(entry.id) === sizeId)
    ) {
      rmItemSizeMap[key].push({
        id: sizeId,
        name: row.size_name || String(sizeId),
      });
    }
  });
  Object.values(rmItemColorMap).forEach((entries) =>
    entries.sort((a, b) => String(a.name).localeCompare(String(b.name))),
  );
  Object.values(rmItemSizeMap).forEach((entries) =>
    entries.sort((a, b) => String(a.name).localeCompare(String(b.name))),
  );
  ...
  itemSizeSetMap.forEach((sizeSet, itemId) => {
    const rows = [...sizeSet]
      .map((sizeId) => ({
        id: sizeId,
        name: sizeNameById.get(sizeId) || String(sizeId),
      }))
      .sort((a, b) => String(a.name).localeCompare(String(b.name)));
    itemSizeMap[String(itemId)] = rows;
  });

  const ruleSizes = (sizes || []).filter((row) =>
    fgSfgSizeIds.has(Number(row.id)),
  );
  const ruleColors = (colors || []).filter((row) =>
    fgSfgColorIds.has(Number(row.id)),
  );
  const fgToSfgMap = {};
  (fgSfgUsageRows || []).forEach((row) => {
    ...
    if (!fgToSfgMap[key].includes(String(sfgId)))
      fgToSfgMap[key].push(String(sfgId));
  });
  ...
  (itemsRaw || [])
    .filter(
      (item) =>
        String(item?.item_type || "").toUpperCase() === "FG" &&
        Boolean(item?.uses_sfg),
    )
    .forEach((item) => {
      ...
      if (!fgCode) return;
      const partType = String(item?.sfg_part_type || "")
        .trim()
        .toUpperCase();
      const suffixes =
        partType === "STEP"
      ...
      const inferred = suffixes
        .map((suffix) =>
          sfgItemIdByCode.get(`${fgCode}_${suffix}`.toLowerCase()),
        )
        .filter(Boolean)
        .map((id) => String(id));
  ...
  (labourRateRuleRows || []).forEach((rule) => {
    ...
    if (
      !Object.prototype.hasOwnProperty.call(
        LABOUR_RATE_RULE_PRECEDENCE,
        applyOn,
      )
    )
      return;
    if (
      !LABOUR_RATE_TYPES.has(
        String(rule?.rate_type || "")
          .trim()
          .toUpperCase(),
      )
    )
    ...
    if (applyOn === "SUBGROUP") {
      ...
      if (!labourRulesBySubgroupId.has(subgroupId))
        labourRulesBySubgroupId.set(subgroupId, []);
    ...
    if (applyOn === "GROUP") {
      ...
      if (!groupId) return;
      if (!labourRulesByGroupId.has(groupId))
        labourRulesByGroupId.set(groupId, []);
  ...
  (items || []).forEach((item) => {
    ...
    const skuRows = (itemSkuRowsByItemId.get(itemId) || [])
      .slice()
      .sort((a, b) =>
        String(a.sku_code || "").localeCompare(String(b.sku_code || "")),
      );
    ...
    skuRows.forEach((skuRow) => {
      ...
      candidates.forEach((rule) => {
        ...
        if (
          !labourId ||
          !deptId ||
          !LABOUR_RATE_TYPES.has(rateType) ||
          rateValue === null ||
          rateValue < 0
        )
          return;
        ...
        if (
          ...
          (precedence === current.precedence && ruleId > current.ruleId)
        ) {
      ...
      bestRuleByCombo.forEach((resolved) => {
        ...
        if (!comboRatesBySize.has(comboKey))
          comboRatesBySize.set(comboKey, new Map());
    ...
    comboRatesBySize.forEach((sizeRateMap, comboKey) => {
      ...
      sizeRateMap.forEach((rateEntries, sizeKey) => {
        const normalizedEntries = Array.isArray(rateEntries)
          ...
          : [];
        const rates = [
          ...new Set(normalizedEntries.map((entry) => entry.rate)),
        ];
        if (rates.length !== 1) return;
        const resolvedRate = String(rates[0]);
        labourRateMap[`${comboKey}:${sizeKey}`] = resolvedRate;
        const allLockedForSize =
          normalizedEntries.length > 0 &&
          normalizedEntries.every((entry) => entry.isLocked);
        if (allLockedForSize)
          labourRateLockedMap[`${comboKey}:${sizeKey}`] = true;
        resolvedBySize.push({
          rate: resolvedRate,
          isLocked: allLockedForSize,
        });
      });
      const uniqueResolvedRates = [
        ...new Set(resolvedBySize.map((entry) => entry.rate).filter(Boolean)),
      ];
      if (uniqueResolvedRates.length === 1) {
        labourRateMap[`${comboKey}:ALL`] = uniqueResolvedRates[0];
        const allLockedForAll =
          resolvedBySize.length > 0 &&
          resolvedBySize.every((entry) => entry.isLocked);
  ...
  const uomNameById = new Map(
    (uoms || []).map((row) => [toNumberOrNull(row.id), row?.name || row?.id]),
  );
  const baseUomIds = [
    ...new Set(
      (items || [])
        .map((item) => toNumberOrNull(item?.base_uom_id))
        .filter(Boolean),
    ),
  ];
  const conversionRows =
    hasUomConversionsTable && baseUomIds.length
  ...
  (conversionRows || []).forEach((row) => {
    ...
    if (!conversionFromSetByBase.has(toId))
      conversionFromSetByBase.set(toId, new Set());
  ...
  (items || []).forEach((item) => {
    ...
    const sortedAllowedRows = [...allowed]
      .map((uomId) => ({
        id: uomId,
        name: uomNameById.get(uomId) || String(uomId),
      }))
      .sort((a, b) => String(a.name).localeCompare(String(b.name)));
    itemOutputUomMap[String(itemId)] = sortedAllowedRows;
  });

  const productionStages = (productionStageRows || [])
    .map((row) => ({
      id: Number(row?.id),
  ...
  return {
    ...
    ruleActionOptions: [{ value: "ADJUST_QTY", label: "bom_rule_adjust_qty" }],
    scopeOptions: [
      { value: "ALL", label: "all" },
...
const listBoms = async (knex, filters = {}, options = {}) => {
  ...
  const level = String(
    filters.bom_type || filters.bomType || filters.level || "",
  ).toUpperCase();
...
const resetPendingBomAfterRejectTx = async (trx, request) => {
  ...
  await trx("erp.bom_header").where({ id: bomId }).update({
    status: "DRAFT",
    approved_by: null,
    approved_at: null,
  });
...
const applyApprovedBomChange = async (trx, request, approverUserId) => {
  ...
  const action = String(payload._action || "").toLowerCase();
  debugBom("applyApprovedBomChange", {
  ...
  if (action === "create" || action === "update") {
    const existingId =
      action === "update" && request.entity_id && request.entity_id !== "NEW"
