...
const normalizeArticleType = (value) => {
  ...
  if (raw === ARTICLE_TYPE.FINISHED || raw === ARTICLE_TYPE.SEMI_FINISHED)
    return raw;
  return "";
};
...
const normalizeScopeInput = ({ payload, t }) => {
  ...
  if (
    !labourSelection.all &&
    (!Array.isArray(labourSelection.labourIds) ||
      !labourSelection.labourIds.length)
  ) {
  ...
  const skuIds = skuIdsInput.length
    ...
    : skuIdFallback
      ? [skuIdFallback]
      : [];
  const subgroupIdsInput = toPositiveIntArray(payload.subgroup_ids);
  const groupIdsInput = toPositiveIntArray(payload.group_ids);
  const subgroupIds = subgroupIdsInput.length
    ? subgroupIdsInput
    : toPositiveIntArray(payload.subgroup_id);
  const groupIds = groupIdsInput.length
    ? groupIdsInput
  ...
  const rateType = String(payload.rate_type || "")
    .trim()
    .toUpperCase();
  ...
  return {
    ...
    subgroupIds: applyOn === APPLY_ON.SUBGROUP ? subgroupIds : [],
    groupId: applyOn === APPLY_ON.GROUP ? groupIds[0] || null : null,
    groupIds: applyOn === APPLY_ON.GROUP ? groupIds : [],
    articleType,
    rateType,
    status: statusRaw,
  };
...
const resolveLabourIds = async ({ db = knex, deptId, labourSelection, t }) => {
  ...
  const selectedLabourIds = [
    ...new Set(
      (Array.isArray(labourSelection?.labourIds)
        ? labourSelection.labourIds
        : []
      )
        .map((id) => Number(id))
        .filter((id) => Number.isInteger(id) && id > 0),
...
const fetchTargetSkus = async ({
  ...
  if (applyOn === APPLY_ON.SKU) {
  ...
  } else if (applyOn === APPLY_ON.SUBGROUP) {
    const requestedSubgroupIds = [
      ...new Set(
        (Array.isArray(subgroupIds) && subgroupIds.length
          ? subgroupIds
          : subgroupId
            ? [subgroupId]
            : []
    ...
    if (requestedSubgroupIds.length) {
      query = query.whereIn("i.subgroup_id", requestedSubgroupIds);
    }
  } else if (applyOn === APPLY_ON.GROUP) {
    ...
    if (requestedGroupIds.length) {
      query = query.whereIn("i.group_id", requestedGroupIds);
    }
  }
...
const fetchExistingRules = async ({ db = knex, labourIds, deptId }) => {
  const dept = Number(deptId || 0);
