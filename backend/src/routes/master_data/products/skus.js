...
const loadOptions = async () => {
  const [items, sizes, grades, colors, packings] = await Promise.all([
    knex("erp.items")
  ...
  return { items, sizes, grades, colors, packings };
};
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const comboKeys = toArray(values.combo_keys).map((value) => String(value || "")).filter(Boolean);
    ...
    await knex.transaction(async (trx) => {
      ...
      const sizeMap = new Map(
        (await trx("erp.sizes").select("id", "name").whereIn("id", sizeIds)).map((row) => [row.id, row.name])
      );
      const gradeMap = new Map(
        (await trx("erp.grades").select("id", "name").whereIn("id", gradeIds)).map((row) => [row.id, row.name])
      );
      const colorList = colorIds.length ? colorIds : [null];
      const packingList = packingIds.length ? packingIds : [null];
      const colorMap = new Map(
        colorIds.length
          ? (await trx("erp.colors").select("id", "name").whereIn("id", colorIds)).map((row) => [row.id, row.name])
          : []
      );
      const packingMap = new Map(
        packingIds.length
          ? (await trx("erp.packing_types").select("id", "name").whereIn("id", packingIds)).map((row) => [row.id, row.name])
          : []
      );
        ...
        for (const grade_id of gradeIds) {
          for (const color_id of colorList) {
            for (const packing_type_id of packingList) {
              ...
              const existing = await findVariant(trx, {
                item_id,
                size_id,
                grade_id,
                color_id,
              ...
              if (!variantId) {
                ...
                await trx("erp.variants")
                  .where({ id: variantId })
                  .update({ sale_rate: resolvedRate });
              }

              const baseSku = buildSkuCode(item ? item.code : "", [
                sizeMap.get(size_id),
                gradeMap.get(grade_id),
                color_id ? colorMap.get(color_id) : null,
                packing_type_id ? packingMap.get(packing_type_id) : null,
              ]);
...
router.post("/:id", async (req, res, next) => {
  ...
  try {
    ...
    await knex.transaction(async (trx) => {
      ...
      const packing = packing_type_id
        ? await trx("erp.packing_types").select("name").where({ id: packing_type_id }).first()
        : null;
