...
const buildComboKey = (sizeId, gradeId, colorId, packingId) =>
  [sizeId || 0, gradeId || 0, colorId || 0, packingId || 0].join("|");

const buildSfgComboKey = (itemId, sizeId, colorId) =>
  ...
  const [items, sizes, grades, colors, packings, subgroups] = await Promise.all(
    [
      ...
      knex("erp.product_subgroups as sg")
        ...
        .join("erp.items as i", "sg.id", "i.subgroup_id")
...
const syncSfgVariantsFromFinished = async (userId) => {
  await knex.transaction(async (trx) => {
    ...
    const sfgItems = await trx("erp.items")
      .select("id", "name", "code")
      .whereIn("id", sfgIds);
    const sfgNameMap = new Map(sfgItems.map((x) => [x.id, x.name]));
    const sfgCodeMap = new Map(sfgItems.map((x) => [x.id, x.code]));

    const existingRows = await trx("erp.variants")
      .select("item_id", "size_id", "color_id")
      .whereIn("item_id", sfgIds)
      .whereNull("grade_id")
      .whereNull("packing_type_id");
    const existingSet = new Set(
      existingRows.map((row) =>
        buildSfgComboKey(row.item_id, row.size_id, row.color_id),
      ),
    );

    const sizeIds = [
      ...new Set(usageRows.map((row) => row.size_id).filter(Boolean)),
    ];
    const colorIds = [
      ...new Set(usageRows.map((row) => row.color_id).filter(Boolean)),
    ];
    ...
    for (const row of usageRows) {
      const comboKey = buildSfgComboKey(
        row.sfg_item_id,
        row.size_id,
        row.color_id,
      );
      ...
      const baseSku = buildSkuCode(base, [
        sizeMap.get(row.size_id),
        row.color_id ? colorMap.get(row.color_id) : null,
        suffix,
      ]);
...
const loadRows = async (filters = {}, itemType = "FG") => {
  let query = knex("erp.variants as v")
    .select(
      ...
      "g.name_ur as grade_name_ur",
      "c.name as color_name",
      "c.name_ur as color_name_ur",
      "p.name as packing_name",
      "p.name_ur as packing_name_ur",
      "k.sku_code",
      "k.barcode",
      "k.is_active as sku_active",
      "ar.status as pending_approval_status",
    ...
    .leftJoin("erp.approval_request as ar", function () {
      this.on("ar.entity_id", "=", knex.raw("CAST(v.id AS TEXT)"))
        .andOn("ar.entity_type", "=", knex.raw("'SKU'"))
        .andOn("ar.status", "=", knex.raw("'PENDING'"))
        .andOn("ar.request_type", "=", knex.raw("'MASTER_DATA_CHANGE'"));
    });
  ...
  if (filters.search && filters.search.trim()) {
    ...
    query.where(function () {
      this.whereRaw("LOWER(k.sku_code) LIKE ?", [search])
        .orWhereRaw("LOWER(i.name) LIKE ?", [search])
        .orWhereRaw("LOWER(s.name) LIKE ?", [search])
        .orWhereRaw("LOWER(g.name) LIKE ?", [search])
...
router.get(
  "/config/:itemId",
  requirePermission("SCREEN", "master_data.products.skus", "navigate"),
  async (req, res, next) => {
    try {
      ...
      const variants = await knex("erp.variants")
        .select("size_id", "grade_id", "color_id", "packing_type_id")
        .where("item_id", itemId);

      const config = {
        size_ids: [...new Set(variants.map((v) => v.size_id).filter(Boolean))],
        grade_ids: [
          ...new Set(variants.map((v) => v.grade_id).filter(Boolean)),
        ],
        color_ids: [
          ...new Set(variants.map((v) => v.color_id).filter(Boolean)),
        ],
        packing_type_ids: [
          ...new Set(variants.map((v) => v.packing_type_id).filter(Boolean)),
        ],
        existing_combinations: variants.map((v) =>
          buildComboKey(v.size_id, v.grade_id, v.color_id, v.packing_type_id),
        ),
      };
...
router.get(
  "/",
  requirePermission("SCREEN", "master_data.products.skus", "view"),
  async (req, res, next) => {
    try {
...
router.post(
  "/",
  requirePermission("SCREEN", "master_data.products.skus", "create"),
  async (req, res) => {
    ...
    try {
      ...
      if (approvalRequired && queuedCount > 0) {
        setApprovalNotice(
          res,
          res.locals.t("variants_sent_approval") ||
    ...
    try {
      ...
      if (process.env.DEBUG_SKU_PERMS === "1") {
        console.log("[SKU PERM DEBUG]", {
          ...
          allowed,
          scopePerms:
            req.user?.permissions?.["SCREEN:master_data.products.skus"] || null,
        });
      }
      if (!allowed && !approvalRequired) {
        throw new HttpError(403, res.locals.t("permission_denied"));
      ...
      if (approvalRequired) {
        ...
        return res.redirect(
          basePath +
            viewQuery +
            "&success=true&msg=" +
            encodeURIComponent(res.locals.t("approval_submitted")),
        );
      }
...
router.post(
  "/:id/toggle",
  requirePermission("SCREEN", "master_data.products.skus", "delete"),
  async (req, res, next) => {
    const id = Number(req.params.id);
    const itemType = req.query.item_type === "SFG" ? "SFG" : "FG";
    const viewQuery = `?item_type=${itemType}`;
...
router.post(
  "/:id/delete",
  requirePermission("SCREEN", "master_data.products.skus", "hard_delete"),
  async (req, res, next) => {
    const id = Number(req.params.id);
    const itemType = req.query.item_type === "SFG" ? "SFG" : "FG";
    const viewQuery = `?item_type=${itemType}`;
