...
router.get(
  "/",
  requirePermission("SCREEN", "master_data.basic_info.uom_conversions", "view"),
  async (req, res, next) => {
    try {
      const canBrowse = res.locals.can(
        "SCREEN",
        "master_data.basic_info.uom_conversions",
        "navigate",
      );
...
router.post(
  "/:id",
  requirePermission("SCREEN", "master_data.basic_info.uom_conversions", "edit"),
  async (req, res, next) => {
