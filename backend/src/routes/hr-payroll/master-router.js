...
const normalizeMode = (value) =>
  String(value || "").toLowerCase() === "exclude" ? "exclude" : "include";
...
const isEmployeesDebugEnabled = (pageConfig) =>
  process.env.DEBUG_HR_EMPLOYEES === "1" &&
  pageConfig?.scopeKey === "hr_payroll.employees";
...
const isLabourRatesSaveDebugEnabled = (pageConfig) =>
  pageConfig?.scopeKey === "hr_payroll.labour_rates";
...
const applyAllowedBranchScopeToQuery = (
  query,
  pageConfig,
  allowedBranchIds,
) => {
  if (!Array.isArray(allowedBranchIds) || !allowedBranchIds.length)
    return query;
  ...
  if (
    pageConfig.branchFilter?.mapTable &&
    pageConfig.branchFilter?.mapKey &&
    pageConfig.branchFilter?.entityKey
  ) {
    const {
      mapTable,
      mapKey,
...
const fetchRows = (pageConfig, options = {}) => {
  ...
  const secondaryValues = sanitizeFilterValues(
    secondary,
    filters.secondaryValues,
  );
  const tertiaryValues = sanitizeFilterValues(tertiary, filters.tertiaryValues);
  const rawBranchIds = Array.isArray(filters.branchValues)
    ? filters.branchValues
        .map((value) => Number(value))
        .filter((value) => Number.isFinite(value))
    : [];
  const allowedSet = allowedBranchIds.length ? new Set(allowedBranchIds) : null;
  const branchIds = allowedSet
    ? rawBranchIds.filter((id) => allowedSet.has(id))
    : rawBranchIds;
      ...
      if (filters.branchMode === "exclude") {
        query = query.whereNotExists(function () {
          this.select(1)
            .from(mapTable)
            .whereRaw(`${mapTable}.${mapKey} = t.${entityKey}`)
            .whereIn(`${mapTable}.${branchKey}`, branchIds);
        });
      } else {
        query = query.whereExists(function () {
          this.select(1)
    ...
    if (field.type === "select") {
      ...
      if (field.multiple === true) {
        ...
        if (rawValue && typeof rawValue === "object") {
          acc[field.name] = Object.values(rawValue)
            .map((value) => String(value || "").trim())
            .filter(Boolean);
          return acc;
        }
...
const normalizeMultiMapValues = (pageConfig, values = {}) => {
  const multiMaps = Array.isArray(pageConfig.multiMaps)
    ? pageConfig.multiMaps
    : [];
...
const createHrMasterRouter = (pageConfig) => {
  ...
  router.get(
    "/",
    requirePermission("SCREEN", pageConfig.scopeKey, "view"),
    async (req, res, next) => {
      try {
        ...
        const applyOnValues = parseList(req.query.apply_on)
          .map((value) =>
            String(value || "")
              .trim()
              .toUpperCase(),
        ...
        const articleTypeValues = parseList(req.query.article_type)
          .map((value) =>
            String(value || "")
              .trim()
              .toUpperCase(),
          ...
          .filter(Boolean);
        const rateViewRaw = String(req.query.rate_view || "")
          .trim()
          .toUpperCase();
        ...
        const getOptionsForFilter = async (config) => {
          ...
          const field = hydrated.fields.find(
            (entry) => entry.name === config.fieldName,
          );
          return Array.isArray(field?.options) ? field.options : [];
        };
        const [primaryOptions, secondaryOptions, tertiaryOptions] =
          await Promise.all([
            getOptionsForFilter(primaryFilter),
            getOptionsForFilter(secondaryFilter),
            getOptionsForFilter(tertiaryFilter),
          ]);
        const filterConfig = {
        ...
        const requiredFilters = Array.isArray(
          pageConfig.listScopeRequiredFilters,
        )
          ? pageConfig.listScopeRequiredFilters
          : [];
        ...
        const configuredMaxRows = Number(hydrated.maxRows || 0);
        ...
        const listScopeMessage =
          ...
          pageConfig.listScopeRequiredMessageKey
  ...
  router.post(
    "/",
    requirePermission("SCREEN", pageConfig.scopeKey, "create"),
    async (req, res, next) => {
      const values = buildValues(pageConfig, req.body);
      const basePath = req.baseUrl;
      const sanitizedValues = pageConfig.sanitizeValues
        ? pageConfig.sanitizeValues(values, req)
        : values;
      ...
      try {
        if (pageConfig.validateValues) {
          const validationError = await pageConfig.validateValues({
          ...
          if (validationError) {
            ...
            logHrValidationDebug(pageConfig, req, "create:validation_error", {
              validationMessage:
                normalized?.message || String(validationError || ""),
        ...
        const branchIds = Array.isArray(sanitizedValues.branch_ids)
          ? sanitizedValues.branch_ids.map(String)
          : [];
        if (pageConfig.branchScoped && !branchIds.length) {
          logEmployeeDebug(pageConfig, req, "create:no_branch_selected", {
            traceId,
          });
          return renderIndexError(
            req,
        ...
        const { branch_ids: branchIdsInsert = [], ...valuesWithoutBranch } =
          sanitizedValues;
        const { persistedValues: rest, mapValues } = normalizeMultiMapValues(
          pageConfig,
          valuesWithoutBranch,
        );
        ...
        await knex.transaction(async (trx) => {
          ...
          if (
            Array.isArray(pageConfig.multiMaps) &&
            pageConfig.multiMaps.length
          ) {
        ...
        console.error("[hr-master:create]", {
          scopeKey: pageConfig.scopeKey,
          error: err.message,
        });
        return renderIndexError(
  ...
  router.post(
    "/:id",
    requirePermission("SCREEN", pageConfig.scopeKey, "edit"),
    async (req, res, next) => {
      ...
      try {
        ...
        if (!existing) {
          logLabourRatesSaveDebug(pageConfig, req, "edit:not_found", {
            traceId,
            id,
          });
          return renderIndexError(
        ...
        if (existing.code) {
          ...
          hasField(pageConfig, "code") ||
          pageConfig.autoCodeFromName
        ) {
          sanitizedValues.code = await generateUniqueCode({
            name: sanitizedValues.name,
            prefix: pageConfig.codePrefix || pageConfig.entityType || "",
        ...
        const nameValue = sanitizedValues.name || "";
        if (codeValue) {
          const codeExists = await knex(pageConfig.table)
            .whereRaw("lower(code) = ?", [codeValue.toLowerCase()])
            .andWhereNot({ id })
            .first();
          if (codeExists) {
            logLabourRatesSaveDebug(pageConfig, req, "edit:duplicate_code", {
              traceId,
              id,
              codeValue,
            });
        ...
        if (nameValue) {
          const nameExists = await knex(pageConfig.table)
            .whereRaw("lower(name) = ?", [nameValue.toLowerCase()])
            .andWhereNot({ id })
            .first();
          if (nameExists) {
            logLabourRatesSaveDebug(pageConfig, req, "edit:duplicate_name", {
              traceId,
              id,
              nameValue,
            });
        ...
        const branchIds = Array.isArray(sanitizedValues.branch_ids)
          ...
          : [];
        if (pageConfig.branchScoped && !branchIds.length) {
          logLabourRatesSaveDebug(pageConfig, req, "edit:no_branch_selected", {
            traceId,
            id,
          });
          return renderIndexError(
        ...
        const { branch_ids: branchIdsUpdate = [], ...valuesWithoutBranch } =
          sanitizedValues;
        const { persistedValues: rest, mapValues } = normalizeMultiMapValues(
          pageConfig,
          valuesWithoutBranch,
        ...
        logLabourRatesSaveDebug(pageConfig, req, "edit:db_update_success", {
          traceId,
          id,
          changed: Array.isArray(changeSet?.changed)
        ...
        console.error("[hr-master:update]", {
          scopeKey: pageConfig.scopeKey,
          id,
          error: err.message,
        });
  ...
  router.post(
    "/:id/toggle",
    requirePermission("SCREEN", pageConfig.scopeKey, "delete"),
    async (req, res, next) => {
      const id = Number(req.params.id);
      if (!id) {
        return next();
      ...
      try {
        ...
        console.error("[hr-master:toggle]", {
          scopeKey: pageConfig.scopeKey,
          id,
          error: err.message,
        });
  ...
  router.post(
    "/:id/delete",
    requirePermission("SCREEN", pageConfig.scopeKey, "hard_delete"),
    async (req, res, next) => {
      const id = Number(req.params.id);
      if (!id) {
        return next();
      ...
      try {
        ...
        const hasStatusField =
          hasField(pageConfig, "status") ||
          Object.prototype.hasOwnProperty.call(existing || {}, "status");
        const hasIsActiveField =
          hasField(pageConfig, "is_active") ||
          Object.prototype.hasOwnProperty.call(existing || {}, "is_active");
        ...
        console.error("[hr-master:hard-delete]", {
          scopeKey: pageConfig.scopeKey,
          id,
          error: err.message,
        });
