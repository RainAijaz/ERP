...
const page = {
  ...
  fields: [
    {
      name: "name",
      label: "name",
      placeholder: "placeholder_employee_name",
      required: true,
    },
    ...
    {
      name: "designation",
      label: "designation_role",
      placeholder: "placeholder_designation_role",
      required: true,
    },
    {
      name: "payroll_type",
  ...
  validateValues: async ({ values, req, isUpdate, id }) => {
    ...
    if (!payrollTypes.has(values.payroll_type))
      return req.res.locals.t("error_invalid_payroll_type");
    if (values.basic_salary === null || Number(values.basic_salary) < 0)
      return req.res.locals.t("error_invalid_salary");
    if (
      !hasTwoDecimalsOrLess(values.basic_salary) ||
      Number(values.basic_salary) > 99999999.99
    )
      return req.res.locals.t("error_invalid_salary_precision");
    if (!values.cnic)
      return {
        field: "cnic",
    ...
    if (values.cnic) {
      const q = knex("erp.employees").whereRaw(
        "regexp_replace(coalesce(cnic, ''), '[^0-9]', '', 'g') = ?",
        [values.cnic],
      );
      ...
      if (dup)
        return {
          field: "cnic",
          message: req.res.locals.t("error_duplicate_cnic"),
        };
    }
    if (values.phone) {
      const q = knex("erp.employees").whereRaw(
  ...
  hasDependencies: async ({ id, knex }) => {
    const checks = await Promise.all([
      ...
      knex("erp.sales_order_header")
        .where({ salesman_employee_id: id })
        .first(),
    ]);
