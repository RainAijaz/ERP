...
router.get(
  "/",
  requirePermission("SCREEN", "administration.permissions", "view"),
  async (req, res, next) => {
    try {
      ...
      const canBrowsePermissions = res.locals.can(
        "SCREEN",
      ...
      if (target_id) {
        if (isUserMode) {
          ...
          const r = await knex("erp.role_templates")
            .where({ id: target_id })
            .first();
      ...
      // 2. Fetch All Scopes
      const allScopes = await knex("erp.permission_scope_registry")
      ...
      const scopeKeySet = new Set(
        allScopes.map((scope) => `${scope.scope_type}:${scope.scope_key}`),
      );
      ...
      if (target_id && canBrowsePermissions) {
        if (isUserMode) {
          const userRow = await knex("erp.users")
            .select("primary_role_id")
            .where({ id: target_id })
            .first();
          const roleId = userRow?.primary_role_id;
          const roleRows = roleId
            ? await knex("erp.role_permissions").where({ role_id: roleId })
            : [];
          const rawPerms = await knex("erp.user_permissions_override").where({
            user_id: target_id,
          });
          ...
          if (staleScopeIds.length > 0) {
            await knex("erp.user_permissions_override")
              ...
              .whereIn("scope_id", staleScopeIds)
              .del();
          ...
          const scopeIds = new Set([
            ...Object.keys(basePermissionsMap),
            ...Object.keys(overridePermissionsMap),
          ]);
          ...
          const rawPerms = await knex("erp.role_permissions").where({
            role_id: target_id,
          });
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const actionKeys = ACTION_KEYS;
      const scopeRows = await trx("erp.permission_scope_registry").select(
        "id",
        "scope_type",
        "scope_key",
      );
      const scopeById = new Map(scopeRows.map((row) => [String(row.id), row]));
      ...
      Object.keys(rights).forEach((key) => {
        ...
        const scopeMeta = scopeById.get(String(scopeId));
        if (!scopeMeta) return;
        if (!getApplicableActionSet(scopeMeta.scope_type).has(action)) return;
      ...
      if (
        !isUserMode &&
        req.user?.primaryRoleId &&
        String(req.user.primaryRoleId) === String(target_id)
      ) {
        const scopeRow = await trx("erp.permission_scope_registry")
          .select("id")
          .where({
            scope_type: "SCREEN",
            scope_key: "administration.permissions",
          })
          .first();

        if (scopeRow?.id) {
          const scopeId = String(scopeRow.id);
          const existing = insertMap[scopeId] || {
            role_id: target_id,
            scope_id: scopeId,
          };
      ...
      if (isUserMode && Object.keys(insertMap).length > 0) {
        const userRow = await trx("erp.users")
          .select("primary_role_id")
          .where({ id: target_id })
          .first();
        const roleId = userRow?.primary_role_id;
      ...
      const flashMessage = lockoutGuarded
        ? "Permissions saved. Access to Permissions was kept to avoid locking out your session."
        : "Permissions saved.";
      writeFlash(res, req.baseUrl, {
        type: "success",
      ...
      res.redirect(
        `/administration/permissions?type=${type}&target_id=${target_id}`,
      );
    } catch (err) {
      if (!committed) {
        await trx.rollback();
        ...
        return res.redirect(
          `/administration/permissions?type=${req.body?.type || "role"}&target_id=${req.body?.target_id || ""}`,
        );
      }
      next(err);
    }
