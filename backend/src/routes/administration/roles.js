...
router.get(
  "/",
  requirePermission("SCREEN", "administration.roles", "navigate"),
  async (req, res, next) => {
    try {
      const canNavigate = res.locals.can(
        "SCREEN",
        "administration.roles",
        "navigate",
      );
      const roles = canNavigate
        ? await knex("erp.role_templates").orderBy("id", "asc")
        : [];
      if (req.accepts("html")) {
...
router.get(
  "/new",
  requirePermission("SCREEN", "administration.roles", "create"),
  (req, res) => {
    if (req.xhr) {
      res.render("administration/roles/form", {
        layout: false,
        role: null,
        title: res.locals.t("add_role"),
...
router.post(
  "/",
  requirePermission("SCREEN", "administration.roles", "create"),
  normalizeFields(["name", "name_ur", "description"]),
  async (req, res, next) => {
    try {
      ...
      const existing = await knex("erp.role_templates")
      ...
      if (req.xhr)
        return res.json({ success: true, redirect: "/administration/roles" });
      res.redirect("/administration/roles");
    } catch (err) {
...
router.get(
  "/:id/edit",
  requirePermission("SCREEN", "administration.roles", "edit"),
  async (req, res, next) => {
    try {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      console.log(
        `[${req.id}] [roles:update] xhr=${req.xhr} id=${req.params.id} body=`,
        req.body,
      );
      const { name, name_ur, description } = req.body;
...
router.post(
  "/:id/toggle",
  requirePermission("SCREEN", "administration.roles", "delete"),
  async (req, res, next) => {
    try {
      const role = await knex("erp.role_templates")
        .where({ id: req.params.id })
...
router.post(
  "/:id/delete",
  requirePermission("SCREEN", "administration.roles", "hard_delete"),
  async (req, res, next) => {
    ...
    try {
      const role = await trx("erp.role_templates")
        ...
        .first();
      if (!role) throw new HttpError(404, "Role not found");
      const userCountRow = await trx("erp.users")
        .where({ primary_role_id: req.params.id })
        .count({ count: "*" })
        .first();
