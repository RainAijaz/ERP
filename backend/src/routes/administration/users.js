...
router.get(
  "/",
  requirePermission("SCREEN", "administration.users", "navigate"),
  async (req, res, next) => {
    try {
...
router.get(
  ["/form", "/form/:id"],
  requirePermission("SCREEN", "administration.users", "edit"),
  async (req, res, next) => {
    ...
    try {
      ...
      if (userId) {
        ...
        const branches = await knex("erp.user_branch").where({
          user_id: userId,
        });
...
router.post(
  "/save",
  requirePermission("SCREEN", "administration.users", "edit"),
  normalizeFields(["name", "name_ur", "username", "email"]),
  async (req, res, next) => {
    ...
    try {
      ...
      if (!id && !password)
        throw new HttpError(400, "Password required for new users");
      ...
      if (id && userData.status === undefined) {
        const existingUser = await trx("erp.users")
          .select("status")
          .where({ id })
          .first();
      ...
      if (id) {
        ...
        const [newUser] = await trx("erp.users")
          .insert(userData)
          .returning("id");
      ...
      await trx.commit();
      if (req.xhr)
        return res.json({ success: true, redirect: "/administration/users" });
...
router.post(
  "/:id/toggle",
  requirePermission("SCREEN", "administration.users", "delete"),
  async (req, res, next) => {
    try {
...
router.post(
  "/:id/delete",
  requirePermission("SCREEN", "administration.users", "hard_delete"),
  async (req, res, next) => {
    const trx = await knex.transaction();
    try {
