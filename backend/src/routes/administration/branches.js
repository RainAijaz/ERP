...
router.get(
  "/",
  requirePermission("SCREEN", "administration.branches", "navigate"),
  async (req, res, next) => {
    try {
      ...
      const branches = canNavigate
        ? await knex("erp.branches").orderBy("id", "asc")
        : [];
      if (req.accepts("html")) {
...
router.get(
  "/new",
  requirePermission("SCREEN", "administration.branches", "create"),
  (req, res) => {
    if (req.xhr) {
      res.render("administration/branches/form", {
        layout: false,
        branch: null,
        title: res.locals.t("add_branch"),
...
router.post(
  "/",
  requirePermission("SCREEN", "administration.branches", "create"),
  normalizeFields(["code", "name", "name_ur", "city"]),
  async (req, res, next) => {
    try {
      console.log(
        `[${req.id}] [branches:create] xhr=${req.xhr} body=`,
        req.body,
      );
      ...
      const existing = await knex("erp.branches")
        ...
        .first();
      if (existing)
        throw new HttpError(400, res.locals.t("error_branch_code_exists"));
...
router.get(
  "/:id/edit",
  requirePermission("SCREEN", "administration.branches", "edit"),
  async (req, res, next) => {
    try {
      ...
      if (req.xhr) {
        res.render("administration/branches/form", {
          layout: false,
          branch,
          title: res.locals.t("edit_branch"),
          csrfToken: res.locals.csrfToken,
        });
      } else {
        res.redirect("/administration/branches");
...
router.post(
  ...
  async (req, res, next) => {
    try {
      console.log(
        `[${req.id}] [branches:update] xhr=${req.xhr} id=${req.params.id} body=`,
        req.body,
      );
...
router.post(
  "/:id/toggle",
  requirePermission("SCREEN", "administration.branches", "delete"),
  async (req, res, next) => {
    try {
      const branch = await knex("erp.branches")
        .where({ id: req.params.id })
...
router.post(
  "/:id/delete",
  requirePermission("SCREEN", "administration.branches", "hard_delete"),
  async (req, res, next) => {
