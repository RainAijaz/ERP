(function () {
  const defaultIsIsoDate = (value) =>
    /^\d{4}-\d{2}-\d{2}$/.test(String(value || "").trim());

  function initReportDateRangePicker(options = {}) {
    const root = options.root || document;
    const showError =
      typeof options.showError === "function"
        ? options.showError
        : (msg) => window.alert(msg);
    const invalidDateRangeMessage = String(
      options.invalidDateRangeMessage || "Invalid date range.",
    );
    const selectDateRangeText = String(
      options.selectDateRangeText || "Select Date Range",
    );
    ...
    const dateRangeFromDisplay = root.querySelector(
      "[data-date-range-from-display]",
    );
    const dateRangeToDisplay = root.querySelector(
      "[data-date-range-to-display]",
    );
    const dateRangeCaption = root.querySelector("[data-date-range-caption]");
    const dateRangeMonth1Label = root.querySelector(
      "[data-date-range-month-1-label]",
    );
    const dateRangeMonth2Label = root.querySelector(
      "[data-date-range-month-2-label]",
    );
    const dateRangeMonth1Grid = root.querySelector(
      "[data-date-range-month-1-grid]",
    );
    const dateRangeMonth2Grid = root.querySelector(
      "[data-date-range-month-2-grid]",
    );
    const dateRangeMonth1PrevBtn = root.querySelector(
      "[data-date-range-month-1-prev]",
    );
    const dateRangeMonth1NextBtn = root.querySelector(
      "[data-date-range-month-1-next]",
    );
    const dateRangeMonth2PrevBtn = root.querySelector(
      "[data-date-range-month-2-prev]",
    );
    const dateRangeMonth2NextBtn = root.querySelector(
      "[data-date-range-month-2-next]",
    );
    ...
    const locale =
      String(options.locale || document.documentElement.lang || "en").trim() ||
      "en";
    const monthNamesFromOptions = Array.isArray(options.monthNames)
      ? options.monthNames
      : null;
    ...
    const monthNames =
      monthNamesFromOptions && monthNamesFromOptions.length === 12
        ? monthNamesFromOptions
        : fallbackMonthNames;
    const getMonthLabel = (dt) => {
      if (!(dt instanceof Date) || Number.isNaN(dt.getTime())) return "";
      try {
        const localized = dt.toLocaleString(locale, { month: "short" });
        if (localized && String(localized).trim()) {
          return `${String(localized).trim()} ${dt.getFullYear()}`;
        }
      } catch (_) {}
    ...
    let openedFromIso = "";
    let openedToIso = "";
    let rangeCursorMonth1 = new Date(
    ...
    const syncDateRangeDisplay = () => {
      ...
      dateRangeInput.value =
        from && to
          ? `${toDisplayShortFromIso(from)} - ${toDisplayShortFromIso(to)}`
          : "";
    };

    const refreshRangeDisplayValues = () => {
      if (dateRangeFromDisplay)
        dateRangeFromDisplay.textContent = rangeStart
          ? toDisplayShortFromDate(rangeStart)
          : "-";
      if (dateRangeToDisplay)
        dateRangeToDisplay.textContent = rangeEnd
          ? toDisplayShortFromDate(rangeEnd)
          : "-";
      ...
      if (dateRangeCaption) {
        if (rangeStart && rangeEnd)
          dateRangeCaption.textContent = `${toDisplayShortFromDate(rangeStart)} - ${toDisplayShortFromDate(rangeEnd)}`;
        else if (rangeStart)
          dateRangeCaption.textContent = `${toDisplayShortFromDate(rangeStart)} - ...`;
    ...
    const handleDateSelection = (boundTo, isoValue) => {
    ...
    const renderMonthGrid = (
      ...
      labelEl,
      boundTo,
      view = { showStart: true, showEnd: true, showRange: true },
    ) => {
      if (!gridEl || !labelEl) return;
      labelEl.textContent = getMonthLabel(monthFirstDay);
      gridEl.innerHTML = "";
      ...
      for (let day = 1; day <= daysInMonth; day += 1) {
        const current = dayStart(
          new Date(monthFirstDay.getFullYear(), monthFirstDay.getMonth(), day),
        );
        const currentIso = toIso(current);
        const isStart =
          view.showStart && rangeStart && toIso(rangeStart) === currentIso;
        const isEnd =
          view.showEnd && rangeEnd && toIso(rangeEnd) === currentIso;
        const inRange =
          view.showRange &&
          hasValidRange() &&
        ...
        btn.type = "button";
        btn.className =
          "inline-flex h-7 w-7 items-center justify-center rounded-md text-xs font-semibold leading-none";
        if (isBlockedByRange) {
          btn.className += " text-slate-300 cursor-not-allowed";
          btn.disabled = true;
        } else if (isStart || isEnd)
          btn.className += " bg-slate-800 text-white";
    ...
    const renderCalendars = () => {
      ...
      const month1View = duplicatedMonth
        ...
        : { showStart: true, showEnd: true, showRange: true };
      const month2View = duplicatedMonth
        ? { showStart: false, showEnd: true, showRange: true }
        : { showStart: true, showEnd: true, showRange: true };
      renderMonthGrid(
        month1,
        dateRangeMonth1Grid,
        dateRangeMonth1Label,
        "from",
        month1View,
      );
      renderMonthGrid(
    ...
    const setDateRange = (from, to) => {
      ...
      if (!isIsoDate(fromVal) || !isIsoDate(toVal) || fromVal > toVal)
        return { ok: false };
    ...
    dateRangeMonth1Grid?.addEventListener("click", (event) => {
    ...
    dateRangeApplyBtn?.addEventListener("click", () => {
      const from = rangeStart ? toIso(rangeStart) : "";
      const to = rangeEnd ? toIso(rangeEnd) : "";
